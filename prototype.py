...
def train_model(
    ...
    for e in tqdm(range(epochs), total=epochs, desc="Epoch ", disable=disable_tqdm,):
        try:
            train_losses, train_accs = util.train(model, opt, imgs, labels, train_idx, batch_size=batch_size, use_cuda=use_cuda, disable_tqdm=disable_tqdm,)
        except KeyboardInterrupt:
            print("Training interrupted!")
            interrupted = True
            
        valid_accs = util.test(model,imgs,labels,valid_idx,use_cuda,desc="Validation ", disable_tqdm=disable_tqdm,)
...
def test_model(
    ...
    test_accs = util.test(best_model, test_imgs, test_labels, list(range(len(test_labels))), use_cuda, desc="Test ", disable_tqdm=disable_tqdm,)
